...
class GrowthOptimization(object):
    ...
    bestAnswer = 1e16
    def __init__(self):
        growthRate = np.zeros((12,6))
        self.grshape = growthRate.shape
        growthRate[0,:3] = 0.18,0.23,0.06
        growthRate[1,:3] = 0.19,0.14,0.05
    ...
    def fitness(self,x):
        ...
        if f is None:
            try: 
                growthFull = np.zeros((12,6))
                growthRatesMain = np.zeros((12,3))
                growthRatesCross = np.zeros((12,3))
